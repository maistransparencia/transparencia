...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
      ...
      {/* Container do Modal */}
      <div className="relative z-[10001] w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl transition-all">
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 cursor-pointer rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700"
          aria-label="Fechar"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        {sentSuccess ? (
          <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 text-center">
          ...
          <div className="mt-6 space-y-4">
            {/* Benefícios Rápidos alinhados com a cor da marca #5a72a8 */}
            ...
            {/* Formulário Magic Link por E-mail */}
            <form onSubmit={handleSendMagicLink} className="space-y-3">
              ...
              <button
                ...
                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#5a72a8] py-2.5 font-semibold text-white text-xs shadow-xs transition-colors hover:bg-[#4a5f8c] disabled:cursor-not-allowed disabled:opacity-50"
              >
                <span>
