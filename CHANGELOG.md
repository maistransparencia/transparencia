...
* **Série Histórica Plurianual do Patrimônio Financeiro da Previdência (`fct_caprem_patrimonio_historico_metricas` e `@transparencia/db`):** Materialização da evolução patrimonial consolidada do RPPS (CAPREM) desde 2021 até o exercício corrente com base na Matriz de Saldos Contábeis (MSC Patrimonial / SICONFI), agregando caixa disponível (`111%`) e investimentos de curto e longo prazo (`114%` e `1213%`). Incorpora detecção determinística de anomalias contábeis de reporte (`inconsistencia_declaracao_flag`) e mecanismo de quebra de série metodológica (`variacao_abs = null` e `variacao_pct = null`), prevenindo distorções artificiais em exercícios com omissão ou inversão de saldo de aplicações pela Prefeitura. Disponibiliza a consulta atômica `getCapremPatrimonioHistoricoMetrics` e o enriquecimento do DTO `CapremActuarialTrendDTO` em `@transparencia/db`.
...
* **Mart Analítico de Patrimônio Histórico do RPPS (`fct_caprem_patrimonio_historico_metricas`):** Modelo de métricas com contrato estrito (`enforced: true`), particionamento por competência de encerramento (`max(mes_referencia) over (partition by portal_slug, ano)`), segregação física de caixas e aplicações, macro genérica de integridade `test_positive_values` e testes de paridade unitários e funcionais.
* **Captura de Aplicações de Longo Prazo da Previdência (`stg_siconfi_msc_patrimonial`):** Expansão do filtro contábil para contemplar títulos e aplicações financeiras a longo prazo (`1213%`) para o órgão previdenciário municipal (`10132`), assegurando a correta apuração do patrimônio financeiro consolidado em 2026.
